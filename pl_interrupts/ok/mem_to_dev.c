...
static void dma_buffer_done(void*arg)
{
    ...
    data = readl_relaxed(p+2);
    printk("addr-p=0x%x,data=0x%x\n",p,data);	
    printk("addr-p+2=0x%x,data=0x%x\n",p+2,data);	
...
static int dma_init(struct device *dev,struct dma_test_data *dma_data,dma_addr_t dev_addr,void __iomem *vaddr)
{
    dma_cap_mask_t mask;
    dma_cap_zero(mask);
    dma_cap_set(DMA_SLAVE, mask);
    dma_data->test_ch=dma_request_channel(mask,filter, "dma0chan3");
    if(!dma_data->test_ch)
    {
	...
	printk("dma_data->src_dma_buf:0x%x\n",dma_data->src_dma_buf);
    memset(dma_data->src_buf,0xff,38);
	...
	dma_data->src_buf=kmalloc(32, GFP_KERNEL);
	memset(dma_data->src_buf,0x00,32);

#endif

	struct dma_slave_config test_config;

    test_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES,
    test_config.dst_maxburst = 1,
    test_config.direction= DMA_MEM_TO_DEV,
//    test_config.dst_addr=dma_data->dst_dma_buf;
	test_config.dst_addr=dev_addr+0x08;
...
//    dma_data->tx_desc=dmaengine_prep_slave_single(dma_data->test_ch,dma_data->src_dma_buf,1,DMA_MEM_TO_DEV,DMA_PREP_INTERRUPT|DMA_CTRL_ACK);
	struct scatterlist sg;
//	sg_init_table(&sg, 1);
#if 0
	sg_dma_address(&sg) = dma_data->src_dma_buf;
	sg_dma_len(&sg) = PAGE_SIZE;
	
	int sg_src_len=dma_map_sg(dev, &sg, 1, DMA_TO_DEVICE);	
	if(sg_src_len!= 1) {
		dev_dbg(dev, "unable to map TX DMA\n");
		return -EBUSY;
	}
#else
	dma_data->src_buf[0]=0xff;
	dma_data->src_buf[1]=0xff;
	dma_data->src_buf[2]=0xff;
	
	/* Queue the DMA data transfer. */
	sg_init_one(&sg, dma_data->src_buf, 2);
	dma_map_sg(dev, &sg, 1, DMA_TO_DEVICE);
#endif

//	dma_data->tx_desc=dmaengine_prep_dma_cyclic(dma_data->test_ch,sg_dma_address(&sg),sg_dma_len(&sg),sg_dma_len(&sg)/1,DMA_MEM_TO_DEV,DMA_PREP_INTERRUPT|DMA_CTRL_ACK);
	dma_data->tx_desc=dmaengine_prep_slave_sg(dma_data->test_ch,&sg,1,DMA_MEM_TO_DEV,DMA_PREP_INTERRUPT|DMA_CTRL_ACK);
//	return 0;
	...
	printk("dma_data->cookie=%d\n",dma_data->cookie);

//    dmaengine_submit(dma_data->tx_desc);
//    dma_sync_single_for_device(dma_data->test_ch->device->dev, dma_data->src_dma_buf,1, DMA_TO_DEVICE);
...
#if 1
	int *p=addr;
	writel_relaxed(0x0, p+2);
	dma_buffer_done(NULL);
#endif
